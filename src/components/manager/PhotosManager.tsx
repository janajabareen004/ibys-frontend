...
/**
 * Site Photos manager. Rendered standalone at /manager/photos (with its own
 * header) and embedded inside the Upload Center's "Photos" tab (header hidden).
 */
export function PhotosManager({ showHeader = true }: { showHeader?: boolean }) {
  ...
  const newButton = (
    <Button size="sm" onClick={() => setOpen(true)}><Plus className="h-4 w-4" />{t("manager.pm.photos.new")}</Button>
  );
