...
import { managerMutations } from "@/api/managerApi";
import type { Employee } from "@/mocks/mockManagerService";
...
function Page() {
  ...
  const handleAvailabilityChange = async (id: string, availability: Employee["availability"]) => {
    if (updatingId) return;
    setUpdatingId(id);
    try {
      await managerMutations.updateEmployeeAvailability(id, availability);
      notifySuccess(t("manager.team.statusUpdated") as string);
      await refetch();
    } catch {
      notifyError(t("manager.team.statusUpdateFailed") as string);
    } finally {
      setUpdatingId(null);
    }
  };
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        ...
                        >
                          {t("manager.team.editStatus")}
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onSelect={() => handleAvailabilityChange(e.id, "available")}>
                          {t("manager.team.availabilityStates.available")}
                        </DropdownMenuItem>
                        <DropdownMenuItem onSelect={() => handleAvailabilityChange(e.id, "busy")}>
                          {t("manager.team.availabilityStates.busy")}
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
