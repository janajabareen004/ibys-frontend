...
import type { CompanyNotificationCategory } from "@/mocks/mockCompanyService";
...
function Page() {
  ...
  const { data, loading } = useCompanyNotifications();
  ...
  return (
    <RoleGuard allow="BUILDING_COMPANY">
      <PageHeader
        ...
        actions={<Button size="sm" variant="outline"><CheckCheck className="h-4 w-4" />{t("company.notifications.markAllRead")}</Button>}
      />
